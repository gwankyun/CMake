...
:program:`ctest` 运行由 ``CTestTestfile.cmake`` 文件定义的测试，该文件通常位于由
:manual:`cmake(1)` 生成的构建树中：

.. code-block:: shell

  ctest [<options>...] [--test-dir <path-to-build>] [-- <test-args>...]

如果所有测试通过，退出码为 ``0``。在以下情况下退出码为非零：

* 发生错误，例如处理无效参数失败。

* .. versionadded:: 4.4
    未提供参数且不存在 ``CTestTestfile.cmake``。

* 给定了 :ctest-option:`--no-tests=error` 参数且未执行任何测试，例如当 ``CTestTestfile.cmake``
  未定义任何测试，或给定参数排除了所有测试时。
...
.. option:: --preset <preset>, --preset=<preset>
 ...
 使用测试\ :manual:`预设 <cmake-presets(7)>`\ 来指定测试选项。项目二进制目录由
 :preset:`testPresets.configurePreset` 键推断得出。

 .. versionadded:: 3.30
   :ctest-option:`--test-dir` 选项可以指定与 :preset:`testPresets.configurePreset`
   键所指定不同的二进制目录。

 .. versionchanged:: 4.4
    如果指定了 :ctest-option:`--presets-file`，则不要求 ``CMakePresets.json``
    或 ``CMakeUserPresets.json`` 存在。否则，它们必须存在于顶层源目录中。在之前的版本中，\
    这是严格要求的。
...
.. option:: --presets-file <file>, --presets-file=<file>
 ...
 从给定的 ``<file>`` 中读取\ :manual:`预设 <cmake-presets(7)>`。指定的路径可以是绝对路径\
 或相对于当前工作目录的路径。如果给定 ``--presets-file``，则 ``CMakePresets.json`` 和
 ``CMakeUserPresets.json`` 中定义的预设将被忽略。

.. option:: --list-presets

 列出可用的测试预设。当前工作目录必须包含\ ``CMakePresets.json``\ 和\ ``CMakeUserPresets.json``。

 .. versionchanged:: 4.4
   如果指定了 :ctest-option:`--presets-file`，则不要求 ``CMakePresets.json`` 或
   ``CMakeUserPresets.json`` 存在，且仅会列出给定 ``<file>`` 中定义的预设。否则，\
...
.. option:: --test-load <level>
 ...
 在并行运行测试时（例如使用\ :ctest-option:`-j`），当测试可能导致CPU负载超过给定阈\
 值时，尽量不要启动测试。
...
.. option:: --output-junit <file>
 ...
 存在，它将被覆盖。如果使用\ :ctest-option:`-S`\ 选项运行仪表板脚本，则使用\
...
.. option:: -N, --show-only[=<format>]
 ...
 这个选项告诉CTest列出将要运行但没有实际运行的测试。与\ :ctest-option:`-R`\ 和\
 :ctest-option:`-E`\ 选项一起使用。
...
.. option:: --tests-from-file <filename>
 ...
 任何测试名称的行将被忽略。此选项可以与\ :ctest-option:`-R`、\ :ctest-option:`-E`、\ :ctest-option:`-L`\ 或\ :ctest-option:`-LE`\ 等\
 其他选项组合使用。
...
.. option:: --exclude-from-file <filename>
 ...
 不完全匹配任何测试名称的行将被忽略。此选项可以与\ :ctest-option:`-R`、\ :ctest-option:`-E`、\ :ctest-option:`-L`\ 或\
 :ctest-option:`-LE`\ 等其他选项组合使用。
...
.. option:: -FS <regex>, --fixture-exclude-setup <regex>
 ...
 与\ :ctest-option:`-FA`\ 相同，但只排除匹配的设置测试。

.. option:: -FC <regex>, --fixture-exclude-cleanup <regex>

 与\ :ctest-option:`-FA`\ 相同，但只排除匹配的清理测试。
...
.. option:: -U, --union
 ...
 取\ :ctest-option:`-I`\ 和\ :ctest-option:`-R`\ 的并集。

 默认情况下，同时指定\ :ctest-option:`-R`\ 和\ :ctest-option:`-I`\ 时，将\
...
.. option:: --schedule-random-seed <seed>
 ...
 此选项用于重现因使用\ :ctest-option:`--schedule-random`\ 选项以随机顺序执行而导致的测试失败。
...
当使用\ :ctest-option:`-L`\ 时，为了将测试包含在测试运行中，每个正则表达式必须匹配\
至少一个标签。使用多个\ :ctest-option:`-L`\ 选项意味着“匹配\ **所有**\ 这些”。

:ctest-option:`-LE`\ 选项的工作原理与\ :ctest-option:`-L`\ 类似，但它排除\
测试而不是包括测试。如果每个正则表达式匹配至少一个标签，则排除测试。

如果一个测试没有附加标签，那么\ :ctest-option:`-L`\ 将永远不会包括该测试，并且\
:ctest-option:`-LE`\ 将永远不会排除该测试。以带标签的测试为例，考虑五个测试，它们\
带有以下标签：
...
.. option:: --build-makeprogram <program-name>
 ...
 指定CMake在配置和构建项目时使用的显式make程序。仅适用于基于\ :ref:`Makefile Generators`\ 和\ :ref:`Ninja Generators`\ 的生成器。
...
.. option:: -M <model>, --test-model <model>
 ...
 ``Nightly``\ 和\ ``Continuous``。\ ``-M``\ 和\ :ctest-option:`-T`\ 的组合类\
 似于\ :ctest-option:`-D`。
...
.. option:: -T <action>, --test-action <action>
 ...
 ``test``\ 等。有关操作的完整列表，请参阅\ `仪表板客户端步骤`_ 。\ :ctest-option:`-M`\
 和\ ``-T``\ 的组合类似于\ :ctest-option:`-D`。
...
``DefaultCTestConfigurationType``
  ...
  没有给\ :program:`ctest`\ 命令\ :ctest-option:`-C`\ 选项时要构建的默认配置。\
...
``TimeOut``
  如果未由\ :prop_test:`TIMEOUT`\ 属性或者\ :ctest-option:`--timeout`\
  标识指定，则为每个测试的默认超时。
...
``TLSVerify``
  ...
  * :module:`CTest`\ 模块变量：\ ``CTEST_TLS_VERIFY``
  ...
  .. versionchanged:: 3.31
    默认为开启。之前，默认是关闭的。用户可以将\ :envvar:`CMAKE_TLS_VERIFY`\ 环境变量设置为\
    ``0``\ 以恢复旧的默认值。
...
``tests``
  ...
  ``config``
    可选字段，用于指定测试将要运行的配置。\
    该字段的值始终与\ :program:`ctest`\ 命令行中指定的\ :ctest-option:`-C`\ 选项相匹配。\
    如果未指定该选项，则此选项将不会出现。
  ``command``
...
高\ :ctest-option:`-j`\ 参数，如果这些测试都使用来自同一资源的一些插槽。此外，这意味着单个测试使用的资源\
...
:prop_test:`RESOURCE_GROUPS`\ 属性告诉CTest测试期望使用什么资源，以对测试有意义的方式进行分组。测\
试本身必须读取\ :ref:`环境变量 <ctest-resource-environment-variables>`，以确定分配给\
每个组的资源。例如，每个组可能对应于测试在执行时将产生的一个进程。

注意，即使测试指定了\ :prop_test:`RESOURCE_GROUPS`\ 属性，如果用户没有传递资源规范文件，该测试仍然\
有可能在没有任何资源分配（并且没有相应的\
:ref:`环境变量 <ctest-resource-environment-variables>`）的情况下运行。通过\
:ctest-option:`--resource-spec-file`\ 命令行参数或\ ``RESOURCE_SPEC_FILE``\ 参数将该文件传递给\
...
:ctest-option:`--resource-spec-file`\ 选项指定，也可以使用\ :command:`ctest_test`\
...
:ctest-option:`--resource-spec-file`、\
...
.. envvar:: CTEST_RESOURCE_GROUP_COUNT
  ...
  * ``CTEST_RESOURCE_GROUP_COUNT=3``
  ...
  只有在给\ :manual:`ctest(1)`\ 指定了\ :ctest-option:`--resource-spec-file`，或者给\
...
:prop_test:`PROCESSORS`\ 测试属性，该属性仍然对CTest的\ :ctest-option:`-j`\
...
当通过\ ``make -j 2 test``\ 调用时，CTest连接到作业服务器，为每个测试获取一个\
令牌，并同时运行最多2个测试。
