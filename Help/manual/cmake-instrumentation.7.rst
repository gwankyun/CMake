...
``data/compile-trace/``
  ...
  the compiler, when the ``compileTrace`` :ref:`option <cmake-instrumentation v1 Query Files>` is
  enabled.
...
``options``
  ...
  the following:
    ...
    ``captureOutput``
      ...
      Enables collection of command output in generated |v1 Snippet File|.
...
片段文件的文件名语法为\ ``<role>-<hash>-<timestamp>.json``，并包含以下数据
  ...
  ``stdout``
    ...
    the ``captureOutput`` :ref:`option <cmake-instrumentation v1 Query Files>` and when ``role`` is one
  ...
  ``stderr``
    ...
    enabled by the ``captureOutput`` :ref:`option <cmake-instrumentation v1 Query Files>` and when
  ...
  ``traceFile``
    ...
    included when the ``compileTrace`` :ref:`option <cmake-instrumentation v1 Query Files>`
    is enabled and  ``role`` is
