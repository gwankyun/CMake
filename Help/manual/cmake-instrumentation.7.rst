...
instrumentation API as part of the `数据收集`_ and `索引`_. A new
...
These files remain in the build tree even after `索引`_ occurs and all
`回调函数`_ are executed, until the next time `索引`_ occurs.
